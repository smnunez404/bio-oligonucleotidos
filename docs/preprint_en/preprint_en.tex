...
The pipeline reproduces three results with external anchors. First, the variant supplies the
...
place their maximum at exactly $+1$ and $-89$ relative to the variant, implying a 91~bp pseudoexon
--- which reproduces, \emph{position for position}, the boundaries that Corradi et
al.~\cite{corradi2023} had already measured by midigene assay in 2023 and that Wang et
al.~\cite{wang2025} later confirmed in size by minigene. Third, on an independent published
oligo-walk with measured efficacy~\cite{kaltak2023} the same scoring places the five known-effective
AONs at ranks 1, 3, 4, 5 and 6 of 32 ($\mathrm{AUC}=0.974$).
...
Assembling this manuscript's reference list --- the last step before submission --- produced two
more of the same kind: the source paper had been misattributed to the wrong first author for five
...
The variant \textbf{c.161-395G>A} has been reported by three independent groups. Corradi et
al.~\cite{corradi2023} first characterised it in 2023 by midigene assay in a European cohort,
reporting 36\,\% residual wild-type RNA and the exact pseudoexon boundaries. Wang et
al.~\cite{wang2025} showed by minigene that it activates three pseudoexons (PE1b 91~bp, PE1c
255~bp, PE1d 265~bp), with 76.4\,\% of the resulting mRNA incorrectly spliced, and found it to be
the most prevalent deep-intronic allele of their Chinese cohort. Huang et al.~\cite{huang2025}
recovered it again in a Taiwanese cohort. None of the three reports an antisense oligonucleotide
against it.
...
\subsection{The variant supplies the donor consensus adenine}
...
\subsection{The predicted pseudoexon reproduces published coordinates, position for position}
\label{sec:coordinates}

SpliceAI~\cite{jaganathan2019} local inference over a 10~kb window predicts a cryptic donor
strengthened at $+1$ ($0.194 \rightarrow 0.560$) and a cryptic acceptor at $-89$
($0.061 \rightarrow 0.271$), implying a pseudoexon of \textbf{91~bp}.

Corradi et al.~\cite{corradi2023} measured this variant with a midigene splice assay in 2023 and
reported the resulting transcript as
\texttt{r.160\_161ins161-484\_161-394}. Converted to offsets relative to the variant at
\texttt{c.161-395}, that published RNA variant states the acceptor is at $395-484 = -89$, the donor
at $395-394 = +1$, and the pseudoexon is $484-394+1 = 91$~nt. \textbf{The prediction matches the
published experiment at both boundaries, not merely in length.} Wang et al.~\cite{wang2025}
independently confirmed the 91~bp size by minigene, as PE1b.

A third cohort provides a second kind of replication. Huang et al.~\cite{huang2025} report SpliceAI
delta scores of $\mathrm{DS\_AG} = 0.21$ and $\mathrm{DS\_DG} = 0.37$ for this variant; our local
inference gives $0.271-0.061 = 0.210$ and $0.560-0.194 = 0.366$. Different group, different
pipeline, different cohort, same deltas to two decimals.
...
\needspace{5\baselineskip}
\textbf{This is a validation, not a discovery.} We state plainly that the coordinates were already
in the literature: Corradi et al.\ published them two years before this project began, and the
project did not know it until the reference list for this preprint was assembled
(Section~\ref{sec:novelty}). What the agreement establishes is that the pipeline recovers a
functionally measured pseudoexon from sequence alone --- which is what a design pipeline needs to
be trusted for, and which is worth more here than a novelty claim would have been.

\textbf{The variant strengthens a site that already exists.} Our own wild-type donor score is
$0.194$, which is not zero, and Huang et al.~\cite{huang2025} queried SpliceVault and found that
both cryptic sites are used in normal transcriptomes, below 3\,\% of transcripts. So the correct
statement is that the base change \emph{reinforces} a weak pre-existing cryptic site --- consistent
with the consensus-adenine observation above --- and not that it creates one \emph{de novo}. Earlier
drafts of this work said ``creates''; that was too strong.
...
retina-trained, and a GTEx model that deliberately excludes neural retina --- agree that the variant strengthens the cryptic
donor, though they disagree on its absolute strength. \textbf{Right:} the
...
four to have caught --- one API call --- and, at the time, the last one found.

\subsection{And the coordinates we thought were ours had been published in 2023}
\label{sec:novelty}

The same bibliography work produced a fifth instance, and the most consequential. The project had
recorded that the source paper reports no cryptic site positions. That is true of Wang et
al.~\cite{wang2025}, and was verified against their text. The project then generalised it to
\emph{nobody} having reported them --- which was never checked, and is false.

A full-text search of the literature for the variant string returns exactly three papers. The
earliest, Corradi et al.~\cite{corradi2023}, measured the variant by midigene assay in September
2023 and published the pseudoexon with both of its boundaries, in the notation quoted in
Section~\ref{sec:coordinates}. The third, Huang et al.~\cite{huang2025}, appeared in October 2025
with a Taiwanese cohort and reports the same SpliceAI deltas we compute.

Two things follow, and they point in opposite directions. The external anchor is \textbf{stronger}
than we had claimed --- position-for-position agreement with an independent functional assay rather
than a coincidence of length. And the mechanistic contribution is \textbf{smaller}: none of the
coordinates are new.

The instructive part is why the earlier check missed it. A web search had been run and returned
nothing --- but it searched for the \emph{mechanism} of the variant rather than for the variant
string in full text. The conclusion drawn at the time --- ``absence of a result is not proof of
absence, so do not claim novelty'' --- was the right conclusion reached for the wrong reason: this
was not irreducible uncertainty, it was a paper nobody had looked for correctly. That reflex to
withhold the claim is the only thing that kept the error out of the published record.
\textbf{The query matters as much as the source.}

One premise did survive the search, and is now checked rather than assumed: \textbf{no antisense
oligonucleotide has been published against this variant}. Corradi et al.\ mention none. Sangermano
et al.~\cite{sangermano2019} --- whose title promises AON correction of \emph{ABCA4} splice defects,
and which was therefore the most dangerous paper for this premise --- designed AONs for six other
variants and none for this one. Neither 2025 cohort reports AON work on it.
...
The scientific contribution of this work is modest and bounded, and one draft narrower than we
first wrote it. The pipeline recovers, from sequence alone, a pseudoexon whose boundaries were
established by functional assay in 2023~\cite{corradi2023}; it does not discover them. What remains
is a reproducible pipeline that reconstructs a measured splicing defect and produces a shortlist no
one has tested.
...
which claim to repeat without checking --- and, as Sections~\ref{sec:misattribution}
and~\ref{sec:novelty} show, which citation never to open and which search never to run.

Two of those failures surfaced only while assembling this reference list, after the adversarial
review had concluded. That timing is itself a result: \textbf{writing the citations audited the
project's premises more effectively than six specialised reviewers had}, because it was the first
step that forced every external claim to be resolved against an identifier rather than against the
project's own memory of it. It is also the cheapest step in the whole project.
